...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const title = isEs
    ? 'Poderes y Mandatos Estratégicos | Mac Consultores'
    : 'Strategic Powers of Attorney | Mac Consultores';
...
export default async function PoderesYMandatosPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const dict = await getDictionary(locale);
  const data = dict?.consular?.poderes_y_mandatos;
  const isEs = locale === 'es';
